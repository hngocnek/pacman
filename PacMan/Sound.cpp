...
void PauseMusic() {
	if (sounds->background.getStatus() == sf::Sound::Playing) {
		sounds->background.pause();  // Pause nhạc nếu đang phát
	}
}

// Hàm để tiếp tục nhạc nền
void ResumeMusic() {
	if (sounds->background.getStatus() != sf::Sound::Playing) {
		sounds->background.play();   // Chạy lại nhạc nền nếu chưa phát
	}
}
